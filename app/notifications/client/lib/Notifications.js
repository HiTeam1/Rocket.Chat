...
class Notifications {
	...
	notifyRoom(room, eventName, data) {	
		webSocketHandler.emitToServer('broadcast', {
			key: 'room',
			room,
			eventName,
			data
	...
	notifyUser(userId, eventName, ...args) {
		// return webSocketHandler.emitToServer('broadcast', {
		// 	key: 'user',
		// 	userId,
		// 	eventName,
		// 	data
	...
	onAll(eventName, callback) {
		return webSocketHandler.registerListener(eventName,callback);
		return this.streamAll.on(eventName, callback);
	}

	onLogged(eventName, callback) {
		return this.onLogin(() => webSocketHandler.registerListener(eventName,callback));
	}
	...
	onRoom(room, eventName, callback) {
		const socketRoom = `${eventName}-${getSocketRoom(room)}`;
	
		webSocketHandler.emitToServer('streamBroadcast',{ room: socketRoom });
		webSocketHandler.registerListener(eventName,callback);
		// if (this.debug === true) {
	...
	unRoom(room, eventName, callback) {
		const socketRoom = getSocketRoom(room)
		webSocketHandler.emitToServer('unstreamBroadcast', { socketRoom,  eventName});
		webSocketHandler.removeListener(eventName);
